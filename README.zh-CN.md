...
原有 15 个维度和 100 权重已经恢复，并把 V5 原生实测作为其直接覆盖维度的证据修正，而不是拿 18/18 代替综合评分。
...
这个分数反映 Aethmere 的平衡目标，不是全行业绝对总榜。换成“记忆基础设施优先”权重时，MemOS 为 8.31、Aethmere 为 8.10（第 3/5）；完整 15 维矩阵、权重敏感性、公式、第一方资料来源和限制见 [EVALUATION.md](EVALUATION.md)，机器可读底稿见 [官网 JSON](https://aethmere.com/evaluation/peer-scorecard-v12.json)。
