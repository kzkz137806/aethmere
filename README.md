...
2026-08-25 的工程快照把 Aethmere 与指定版本的 Graphiti、Letta Code、Mem0、MemOS 分别放入隔离安装和冻结合同中核对。下列数字是对 Aethmere 18 维目标画像的**证据成熟度（满分 100）**，不是通用产品排名：

| 产品快照 | 目标画像证据成熟度 |
|---|---:|
| Aethmere（2026-08-25 快照） | 70.75 |
| Graphiti 0.29.3 | 41.50 |
| Letta Code 0.30.29 | 21.00 |
| Mem0 2.0.18 | 36.50 |
| MemOS 2.0.31 | 39.75 |

Aethmere 的内部工程门已达到 18/18，同行隔离实装覆盖 6/18。另由不同操作者在独立物理 Mac 上完成两次全新运行，80/80 个测试单元和 40/40 个拒绝对照通过；这是公开自编合成基准的跨操作者、跨宿主可复现性结果，不是第三方复现、隐藏评测或产品总榜。可直接对齐的切片中，Aethmere 生命周期为 6/6、冻结检索为 10/10、1/8/32 容量为 150/150、严格持续运行完成 7,500/7,500 并通过；其他产品各有通过、未支持或时序门未通过的情况。版本、逐项结果与不可外推边界见 [EVALUATION.md](EVALUATION.md)。
